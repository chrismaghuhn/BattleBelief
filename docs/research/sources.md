---
document_id: research-sources
title: Quellenbasis
document_type: research
status: accepted
normative: false
version: 2
applies_to:
  - research
  - repository
effective_from: 2026-08-05
supersedes: []
superseded_by: null
owners:
  - maintainer
last_reviewed: 2026-08-05
---

# Quellenbasis

Research erklärt und belegt Entscheidungen, ist aber keine operative
Vertragsquelle. Die folgenden Links führen direkt zu den verwendeten
Primärquellen oder Projektquellen.

## Battle-Engine und Protokoll

- [Pokémon Showdown Simulator](https://github.com/smogon/pokemon-showdown/blob/master/sim/SIMULATOR.md)
- [Pokémon Showdown Simulator Protocol](https://github.com/smogon/pokemon-showdown/blob/master/sim/SIM-PROTOCOL.md)
- [Pokémon Showdown Formatdefinitionen](https://github.com/smogon/pokemon-showdown/blob/master/config/formats.ts)
- [Pokémon Showdown Team Validator](https://github.com/smogon/pokemon-showdown/blob/master/sim/team-validator.ts)
- [`poke-engine`](https://github.com/pmariglia/poke-engine)
- [Foul Play Architekturbericht](https://pmariglia.github.io/posts/foul-play/)

## Search und unvollständige Information

- [Simultaneous-Move MCTS](https://proceedings.neurips.cc/paper_files/paper/2013/file/1579779b98ce9edb98dd85606f2c119d-Paper.pdf)
- [Information Set MCTS](https://eprints.whiterose.ac.uk/id/eprint/75048/1/CowlingPowleyWhitehouse2012.pdf)
- [Monte-Carlo Planning in Large POMDPs](https://papers.nips.cc/paper_files/paper/2010/hash/edfbe1afcf9246bb0d40eb4d8027d90f-Abstract.html)
- [Toward Open Set Recognition](https://pubmed.ncbi.nlm.nih.gov/23682001/)
- [Constraint-guided Open-World Set Materialization](constraint-guided-other-materialization.md)

## Daten, Rating und Speicherung

- [Metamon Parsed Replays](https://huggingface.co/datasets/jakegrigsby/metamon-parsed-replays)
- [Smogon Usage Stats Implementation](https://github.com/Antar1011/Smogon-Usage-Stats)
- [Pokémon Showdown Ladder Help](https://pokemonshowdown.com/pages/ladderhelp)
- [SQLite Appropriate Uses](https://sqlite.org/whentouse.html)
- [DuckDB Parquet](https://duckdb.org/docs/current/data/parquet/overview)

## Repository, Sicherheit und Lizenzen

- [GitHub Flow](https://docs.github.com/en/get-started/using-github/github-flow)
- [GitHub Rulesets](https://docs.github.com/en/repositories/configuring-branches-and-merges-in-your-repository/managing-rulesets/about-rulesets)
- [GitHub Actions Security](https://docs.github.com/en/actions/reference/security/secure-use)
- [Apache License 2.0](https://www.apache.org/licenses/LICENSE-2.0)
- [Creative Commons BY-NC 4.0](https://creativecommons.org/licenses/by-nc/4.0/legalcode.en)
- [RFC 8785 – JSON Canonicalization Scheme](https://www.rfc-editor.org/rfc/rfc8785)

Lokale Arbeitsberichte oder Benutzerpfade werden nicht als öffentliche
Quellenlinks eingetragen. Vor Veröffentlichung eines neuen technischen Claims
wird die konkrete Primärquelle hier ergänzt oder direkt im zugehörigen
Research-Dokument zitiert.
