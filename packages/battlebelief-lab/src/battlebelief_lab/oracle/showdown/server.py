...
_MAX_GENERATED_LOG_INDEX_BYTES = 16 * 1024 * 1024
...
def _remove_generated_log_index(log_index: Path) -> None:
    """Remove the bounded regular file owned by this lifecycle invocation.

    Absence is proven before the pinned server starts, so a file at this exact
    ignored path afterward is owned by the controlled lifecycle. Showdown runs
    Git commands against the copy and does not guarantee that it still
    represents HEAD. We therefore validate its file type, independence from
    the real index, and a strict size bound rather than interpreting it as
    provenance. The real source and build are verified again after cleanup.
    """
    ...
    try:
        if os.path.samefile(log_index, git_index):
            raise OSError("upstream log index aliases the real Git index")
        generated_size = log_index.stat().st_size
        if not 0 < generated_size <= _MAX_GENERATED_LOG_INDEX_BYTES:
            raise ValueError("upstream server log index size differs")
        log_index.unlink()
