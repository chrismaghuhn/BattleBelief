...
def _remove_generated_log_index(log_index: Path) -> None:
    """Remove only the known ignored index generated by the upstream server.

    The initial absence check means this never deletes pre-existing source.  A
    symlink or any failed deletion is source dirtiness, not a cleanup best
    effort.  Other generated paths deliberately remain for the verifier to
    reject fail-closed.
    """
    ...
    try:
        if log_index.read_bytes() != git_index.read_bytes():
            raise OSError("upstream server log index bytes differ")
        log_index.unlink()
    except OSError as error:
        raise OracleProcessError(
            OracleFailureClass.SOURCE_DIRTY,
