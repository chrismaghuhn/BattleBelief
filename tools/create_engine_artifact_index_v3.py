"""Create the six-cell v3 resolved-action-order poke-engine artifact index."""
...
from tools.build_poke_engine_wheel import (  # noqa: E402
    ...
    RESOLVED_ACTION_ORDER_INDEX_SCHEMA_ID,
...
class ArtifactIndexV3Error(RuntimeError):
    """A stable v3 artifact-index closure failure."""


def _fail(message: str) -> NoReturn:
    raise ArtifactIndexV3Error(message)
...
def _strict_canonical(path: Path) -> dict[str, Any]:
    ...
    try:
        raw = path.read_bytes()
        value = json.loads(
            raw, object_pairs_hook=reject_duplicates, parse_constant=reject_nonfinite
        )
...
def _build_cell(
    build: Mapping[str, Any], *, source_digest: str, patch_chain_digest: str, wheel_path: Path
) -> dict[str, Any]:
    ...
    expected_build = {
        "schema_version": 3,
        "schema_id": "urn:battlebelief:schema:manifest:engine-build:v3",
        "source_schema_id": "urn:battlebelief:schema:manifest:engine-source:v3",
        "source_manifest_digest": source_digest,
        "downstream_patch_chain_digest": patch_chain_digest,
    ...
    if distribution != {"name": "poke-engine", "version": RESOLVED_ACTION_ORDER_VERSION}:
        _fail("build distribution identity differs")
    expected_wheel = inspect_wheel(
        wheel_path,
    ...
    return {
        ...
        "features": list(FEATURES),
        "adapter_version": RESOLVED_ACTION_ORDER_ADAPTER_VERSION,
        "release_tag": RESOLVED_ACTION_ORDER_RELEASE_TAG,
        "release_asset_url": (
            "https://github.com/chrismaghuhn/BattleBelief/releases/download/"
            f"{RESOLVED_ACTION_ORDER_RELEASE_TAG}/{wheel['filename']}"
        ),
    }
...
def create_artifact_index_v3(
    *,
    ...
    if source_manifest.get("schema_id") != "urn:battlebelief:schema:manifest:engine-source:v3":
    ...
    if not isinstance(patches, list) or len(patches) != 2:
        _fail("patch chain identity differs")
    patch_chain_digest = manifest_digest(patches)
    source_digest = manifest_digest(source_manifest)
    cells = [
        _build_cell(
            build,
            source_digest=source_digest,
            patch_chain_digest=patch_chain_digest,
    ...
    return {
        "schema_version": 3,
        "schema_id": RESOLVED_ACTION_ORDER_INDEX_SCHEMA_ID,
        "manifest_id": "poke-engine-artifact-index-v3-resolved-action-order",
        "source_schema_id": "urn:battlebelief:schema:manifest:engine-source:v3",
        "build_schema_id": "urn:battlebelief:schema:manifest:engine-build:v3",
        "source_manifest_digest": source_digest,
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        if args.evidence_root is not None:
            evidence_paths = sorted(args.evidence_root.rglob("resolved-order-sentinel-*.json"))
            evidence_by_cell = {
                str(document.get("cell_id")): document
                for path in evidence_paths
                for document in [_strict_canonical(path)]
            }
        index = create_artifact_index_v3(
    ...
    except (ArtifactIndexV3Error, KeyError, ValueError, OSError):
        print("v3 artifact index creation failed", file=sys.stderr)
        return 1
